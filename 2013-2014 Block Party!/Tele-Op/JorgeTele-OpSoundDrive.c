...

{
        ...
        if(joy1Btn(1))
                motor[flagSpinner]=SensorValue[soundSensor];//100;
        else if(joy1Btn(3))
                motor[flagSpinner]=-SensorValue[soundSensor];//-100;
        else
                motor[flagSpinner]=0;
        ...
        if(joystick.joy1_TopHat==0){
                motor[driveLeft] = SensorValue[soundSensor];//100;
              ...
              motor[driveLeft] = (abs(joystick.joy1_y1)/joystick.joy1_y1)*SensorValue[soundSensor];//exponentialJoystick(joystick.joy1_y1)*(abs(joystick.joy1_y1)/joystick.joy1_y1);
        }
        else{
                motor[driveLeft] = 0;
                ...
                motor[driveRight] = (abs(joystick.joy1_y2)/joystick.joy1_y2)*SensorValue[soundSensor];//exponentialJoystick(joystick.joy1_y2)*(abs(joystick.joy1_y2)/joystick.joy1_y2);
        }else{
                motor[driveRight] = 0;
        }
}
...
/*        else if(cubeLiftCount>0&&(down||cubeDropperPos==100)&&time1[T2]>1250){ //Autonomously reset cubeLifter
        ...
        if(joystick.joy2_TopHat==4)
                motor[primaryCubeIntake] = SensorValue[soundSensor];//100;
        else if(joy2Btn(2)){ //Reverse intake. Overrides intake forward
                //HTMCstopCal(HTCOMPASS);
                motor[primaryCubeIntake] = SensorValue[soundSensor];//100;
                intakeOn = false;
                motor[motorA] = -SensorValue[soundSensor];//-100;
                motor[motorB] = -SensorValue[soundSensor];//-100;
        }
...
task main(){
      ...
      nxtDisplayCenteredTextLine(1, "%i", SensorValue[soundSensor]);
                }
        }
}
